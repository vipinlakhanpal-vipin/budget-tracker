...
Last updated: 2026-09-07, app version v3.80.
...
**Recent changes (through v3.80):** Investments: the "Your Investment Records" list is now month-scoped like the dashboard tile -- a record no longer shows when browsing any month before its start_date (previously only the dashboard tile did this; the full list stayed all-time). Redesigned the list as a real table (FD/Investment Details, Interest/SIP, Start Date, End Date, Status with quick-set buttons, Gained, Edit/Delete) instead of stacked cards, and moved the Active/Matured/Closed/All filter buttons up next to the "Your Investment Records" heading itself. Also removed the now-redundant mini version badge next to the month selector, since the version already shows in the top bar.

**Recent changes (through v3.79):** Investments: fixed the Save Changes/Cancel buttons stacking on top of each other instead of sitting side by side (a CSS flex-direction inherited from the shared field class). Added an explicit Edit (pencil) icon and one-tap Active/Matured/Closed status buttons directly on each row in Your Investment Records, so status can be changed without opening the edit form at all. Fixed a bug where clicking a record in Your Investment Records did nothing visible -- it quietly loaded the entry into the edit form's state but never actually opened that form, so the Status dropdown (Active/Matured/Closed) was unreachable no matter how you clicked. Clicking a record now opens the edit form as expected. Root-caused and fixed the real reason the app was showing a stuck loading screen since v3.75: the v3.75 edit that added the LoadingScreen component accidentally left `export default` attached to it instead of to the main `App` function, so the build's entry point rendered only the loading stub -- the entire real app (login, dashboard, everything) was silently tree-shaken out of the production bundle and never ran, no matter how the page loaded or how long you waited. Restored `export default` to `App`. (The 3.76 getSession() safety-net timeout was correct but never had a chance to run, since App itself wasn't being rendered -- it's included in this fix.) Found and fixed the actual cause of the reported stuck loading screen during testing: the initial `supabase.auth.getSession()` call can, under some conditions, hang and never resolve, which left the app permanently stuck on the loading screen with no way forward. Added a 10-second safety-net timeout that falls through to the Login screen if that call hasn't settled by then (a session that does arrive later is still picked up normally). Fixed the "My Investments" dashboard tile so it's month-scoped -- an investment no longer counts toward (or appears in the breakdown of) a month before it existed, e.g. an FD opened in July no longer shows up while browsing May; the full Investment Records list stays all-time on purpose. Added a "No active investments" label on that tile instead of a bare $0 when everything active has been closed out. Added an Active/Closed/All filter to the Investment Records list so a matured FD can be marked Closed and still be found later under the Closed filter. Diagnosed the reported white-screen-on-load issue: (1) confirmed that right after a new version deploys, a browser tab with an already-cached page can request a JS/CSS bundle that no longer exists on the server (each deploy's filenames change and old ones aren't kept around) and silently fail to boot -- added a one-time auto-reload in index.html that detects this and recovers automatically; (2) replaced the plain "Loading..." text shown during the initial sign-in/account check with a spinner plus, after 7 seconds, a note that the wait can be normal if the database is waking up from being idle, so a slow load reads as "still working" instead of "broken." Added SMS/email transaction auto-capture backend (webhook_secret column, api/ingest-transaction.js ingestion endpoint using Claude Haiku to parse and categorize forwarded SMS/email text, compatible with Android auto-forwarding apps, an iOS Share Sheet shortcut, and Twilio's inbound-SMS webhook for iOS forward-to-a-number setups, with a soft 10-minute dedupe window) and a matching AutomationSettings panel in Account settings to view/copy/regenerate the webhook secret; a new "SMS & Email Auto-Import" E-Manual topic documents all three setup paths. Added a flashing "New Update Available" message next to the refresh button on desktop and above the Refresh tab in the mobile bottom nav, alongside the existing red badge dot. Swapped the mobile Dashboard tab's house-shaped icon for a proper LayoutDashboard icon. Added Note and Attach Document buttons to the Investments add-form (the Income form already had this) so a note and source document can be kept against each entry. Investments can now be marked "Closed" (status dropdown already existed) and closed investments are excluded from the My Investments dashboard total while remaining visible in the list, labeled Closed, for historic reference -- lets a matured FD be closed out and a renewed term re-entered without double-counting. Fixed the Credit Card/Debit Card/Bank Account tile breakdown lists (introduced in v3.72) so tapping the list icon shows the actual itemized transactions instead of an empty list -- the breakdown lookup now normalizes payment_source/payment_bank the same way the summary tiles do. The AI category-suggestion feature on the Regular/Fixed expense forms now fires on a 500ms debounce as you type instead of waiting for the field to lose focus, cutting the perceived wait for the suggested category. A stray leftover-debugging text fragment in a src/version.js code comment was removed. Dashboard summary tiles (Spent so far, Combined income, Combined expenses, My Investments, and the Credit Card/Debit Card/Bank Account tiles) now have a small list-icon button that opens a modal listing the individual line items behind that total, so a single aggregate number can be checked against what makes it up instead of taken on faith -- Remaining/Net are deliberately left out since they're differences, not sums, with no item list to show. The Delete Account section in Account Settings now sits inside a red-tinted danger-zone box so it reads as clearly distinct from the rest of the settings form. AdminConsole Group Accounts table redesigned (member details only, Plan as its own column); Sign Out bug root-caused and fixed (desktop Profile trigger was sharing a ref with the hidden mobile bottom-nav Profile button, so its dropdown always measured a zero-size element and rendered off-screen -- desktop now uses its own dedicated ref); clear-chat confirm banner moved to render inline directly below the Aria chat input instead of as a document.body-portaled floating banner. New Account sub-tab in Settings (first tab, visible to all users) with Full name/Phone/Location fields reusing the same auto-save state as the profile dropdown, plus read-only email and Owner/Member-since info; Delete My Account moved here from the Currency tab, which now only shows Currency. A full desktop + code-level mobile QA pass found no console errors or broken flows. Known gaps before a Google Play submission: no payment processor wired up yet (the $39/year Premium plan is only a manual Free/Paid toggle in Admin Console) -- Google Play requires Play Billing for any in-app digital subscription purchase), no service worker/offline support, and no /.well-known/assetlinks.json yet (added at TWA-wrapping time).
