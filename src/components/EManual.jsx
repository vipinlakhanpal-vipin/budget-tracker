...
const MANUAL_TOPICS = [
  {
    "key": "updates",
    "title": "What's New",
      "body": "Latest updates (Aug 11, 2026): Hearth now has Free and Paid group account plans -- Free includes Income, Regular Expenses, and Reports; Paid adds Fixed Expenses, Savings, Investments, and Aria. A locked section shows a short \"Premium feature\" prompt linking to Support. You can now delete your own account from Settings › App › Account, and there's now a Terms of Service alongside the Privacy Policy in the footer and in Settings. The footer's \"Suggestion\" link is now \"Support\" -- pick a topic tag or two and describe what's going on, and it goes straight to the app owner. The 10 dashboard summary tiles (Monthly Budget through My Investments) each get their own subtle color now instead of one flat surface. On desktop, Income, Fixed Expenses, Regular Expenses, Savings, and Investments have a thin left rail (Add/View/Charts) so you see one thing at a time instead of everything stacked at once, and picking Charts now shows it at full width instead of a narrow side column. Mobile has the same Add/View/Charts switcher as pills at the top of each section. The top tab bar (Dashboard through Help) is one rounded pill capsule instead of separate solid buttons. The Admin Console's Households tab is now called Group Accounts and shows each group's member emails, plan, and status in a cleaner table layout so admins can tell accounts apart at a glance. Sign Out now works reliably from every menu. The \"Clear chat\" confirmation now appears right below the chat input box instead of floating at the bottom of the screen, so it's easier to spot and act on.",
  },
  {
    "key": "home",
    "title": "Dashboard",
  ...
  },
  {
    ...
    "body": "Below the Regular Expenses form, upload a photo of a receipt (or a screenshot/sheet listing several expenses) and Claude will read it for you. You'll see an editable review list first -- fix anything that looks wrong, untick what you don't want, then add only what you confirm. Nothing is saved automatically." }, { "key": "smsimport", "title": "SMS & Email Auto-Import", "body": "Bank SMS and email alerts about card or account transactions can become expenses automatically, with AI picking the category, instead of you typing each one in by hand. Turn it on once from Settings › Account, where a private household automation secret is shown with Copy and Regenerate buttons -- keep it out of anyone else's hands, since it lets whoever has it add expenses to your household; regenerating it breaks anything already configured until you update it with the new value. On Android this is fully automatic: install a free automation app such as MacroDroid, grant it SMS-read permission, then create a rule with an SMS Received trigger (filtered to your bank's sender) and an HTTP Request action that POSTs to your household's import address with your secret and the message body -- once saved, every future bank text is picked up with zero taps. Concretely, so you can set this up yourself without any developer help: in MacroDroid tap the + to create a new Macro, set the Trigger to SMS Received (pick \"Specific Sender\" and enter your bank's sender ID if you only want bank alerts triggering it), then add an Action of type HTTP Request with Method POST, URL https://budget-tracker-tau-liart.vercel.app/api/ingest-transaction, header Content-Type: application/json, and a JSON body with two fields -- secret set to your household's automation secret (copy it from Settings › Account), and text set to the incoming SMS text (MacroDroid offers this as a variable, such as [sms_body], when you tap into the body field to insert one). Save the Macro with any name you like and grant the SMS permission MacroDroid asks for -- from then on every matching text is sent in and added automatically, no further taps needed. Apple does not let any app read SMS content in the background, and the obvious workaround -- building your own Shortcut that shares a text in and posts it straight to your import address -- turns out to be too fragile for most people to get working reliably (the Shortcuts app's request-body editor has real quirks where a field can look correctly filled in on screen and still arrive empty on the server, with no clear error pointing at why). Two options that work smoothly on iPhone instead, no fiddly setup required: type the transaction in by hand, which only takes a few seconds, or screenshot the bank text and upload that screenshot through Scan a Receipt (in Regular Expenses or Fixed Expenses) -- the same AI that reads a receipt photo reads a screenshot of a bank text just as well. If you want true zero-tap automation and don't mind a small paid step, forward the message to a dedicated number from a third-party SMS-receiving service such as Twilio, whose webhook you point at the same import address -- that route needs you to sign up for that service yourself, since it's a paid number outside this app. Transactions that arrive by email instead (an online purchase receipt, for instance) are fully automatic on both iPhone and Android: add a forwarding rule in your mail app that sends those emails to your import address, and each one is read and added the same way, no tapping required. Whichever path added it, every auto-imported expense still lands in your normal expense list afterward, so anything the AI got wrong -- a mis-picked category, an odd merchant name -- is a normal edit away, the same as anything you'd entered by hand."
  },
  {
    "key": "income",
    "title": "Income",
  ...
  },
  {
    "key": "settings",
    "title": "Settings",
    "body": "Has its own sub-tabs. Currency covers your household's chosen currency (renaming the app/household name itself happens right in the header now -- click the title next to the logo, owners only), plus an Account section with links to the Terms of Service and Privacy Policy and a Delete My Account button if you ever want to leave for good. Smart Budget always follows whichever month you're viewing on the dashboard (change the Month field there to set or review a different month instead) and covers your overall monthly cap for that month, plus an optional \"Budget for Per Category\" section below it and how this month's spending compares to those caps (you'll get a notification in the bell icon if you go over). Groups & Category lets you create Groups (e.g. Subscriptions, Etisalat) and organize your categories under them -- or add, rename, and remove ungrouped categories as before. Users (owners only) covers household members and invites -- see below. Admin Console (owners only) covers members and invites. Every field auto-saves as you edit -- there's no Save button to click."
  },
  {
    "key": "notifications",
    "title": "Notifications",
