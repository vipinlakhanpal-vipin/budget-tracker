...
{ id: 'amber', label: 'Amber', color: '#b45309' },
  ...
  const tileCategoryLine = (type, key) => {
    ...
    const total = sorted.reduce((s, [, amt]) => s + a...) (the original approach). Frame A
  // (.sticky-dashboard-frame, which topRef points into) is `position:
  // sticky; top: 0`, so once the page is scrolled down even a little, that
  // element is already sitting at the top of the viewport from the
  // browser's point of view -- scrollIntoView sees it as "already in view"
...
function ReportHtmlView({ data }) {
  ...
  return (
    <div className="wrap">
      ...
      {/* v3.36: reverted the v3.33 "render inline instead of portal"
          change -- the live-state diagnostic (readout next to the
          version badge) proved clicks and React state updates were
          never actually broken, so the touch-cancel theory that
          motivated moving away from the portal was wrong. The real bug:
          .wrap has overflow-x:clip, which (per the CSS spec quirk
          already documented elsewhere in this file) silently makes
          overflow-y non-visible too -- and that clips the PAINTING of
          any position:fixed descendant to .wrap's own box, even though
          fixed positioning escapes its LAYOUT influence. Portaling
          straight to document.body (below, same as desktop already
          does) sidesteps that clipping ancestor entirely. */}
      ...
      <div className="top-bar" ref={topRef}>
        <div className="top-bar-row">
          ...
          <button
            ...
            title={updateAvailable ? `v${latestVersion} available -- click to refresh` : 'Refresh app'}
...
<span className="corner-profile-label" title="This shows the page load time -- if content changes without you clicking refresh, check whether this time also changed. If it did, your browser silently reloaded the tab (e.g. an inactive tab being reclaimed). If it did not, that\'s unexpected and worth reporting.">{displayNameForEmail(session.user.email)} | {formatVersionBadge().replace(' ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¿ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ½ ', ' | ')} {'\u00b7'} {loadedAt.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
              ...
              {profileMenuOpen && createPortal(
                <div className="profile-dropdown" ref={profileDropdownRef} style={isMobile ? { position: 'fixed', top: 'auto', left: 'auto', right: 12, width: 260, maxWidth: '85vw', bottom: 'calc(78px + env(safe-area-inset-bottom) + 8px)', maxHeight: '60vh', overflowY: 'auto', zIndex: 500 } : { position: 'fixed', top: profileDropdownPos?.top ?? 60, right: profileDropdownPos?.right ?? 12, zIndex: 500 }}>
          ...
          <div className="action-row-teal action-row-left">
            ...
            <button
              ...
              Fixed Expenses
              {!isPaidPlan && <Lock size={9} className="rail-plan-lock" style={{ position: 'static', marginLeft: 4, verticalAlign: 1 }} />}
            </button>
            <button
              type="button"
              data-tour="nav-add"
            ...
            <span className="chat-fab-badge-title chat-fab-badge-below tab-hide-mobile">Aria</span>
                  <span className="chat-fab-greeting tab-hide-mobile">Hey, I'm Aria. Happy to help<br />whenever you're ready</span>
                </>
              )}
              {chatOpen && (() => {
                const chatWindowEl = (
